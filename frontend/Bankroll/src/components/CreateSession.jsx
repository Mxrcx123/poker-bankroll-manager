...
// ─────────────────────────────────────────────────────────────────────────────
// Props:
//   platforms   — Array von { id, name } aus GET /platform/all
//   onSuccess(formData) — async, wird awaited; wirft bei Fehler
//   onCancel()          — optional
// ─────────────────────────────────────────────────────────────────────────────

export default function CreateSession({ platforms = [], onSuccess, onCancel }) {
  const [mode,     setMode]     = useState("cashgame");
  ...
  // Gemeinsame Felder
  const [platform, setPlatform] = useState("");   // platform_id (integer als string)
  ...
  const currentFields = () => ({ buy_in, cash_out, fee, rebuys, addons, winnings });
  ...
  // ── Formular abschicken — ruft onSuccess mit echten Daten auf ──────────────
  async function handleSubmit() {
    const allTouched = { buy_in: true, cash_out: true, winnings: true };
    ...
    try {
      await onSuccess({
        game_mode:   mode,
        platform_id: platform ? parseInt(platform, 10) : null,
        notes:       notes.trim() || null,
        buy_in:      parseNum(buy_in),
        // Cashgame
        cash_out:    mode === "cashgame" ? parseNum(cash_out) : undefined,
        // Turnier
        fee:         mode === "tournament" ? parseNum(fee)     : undefined,
        rebuys:      mode === "tournament" ? parseNum(rebuys)  : undefined,
        addons:      mode === "tournament" ? parseNum(addons)  : undefined,
        winnings:    mode === "tournament" ? parseNum(winnings): undefined,
      });
  ...
  const selectStyle = {
    ...
    borderRadius: "8px", color: profit ? COLORS.text : COLORS.textMuted,
    ...
    return (
      <div style={{
        ...
        <div style={{ fontSize: "13px", color: COLORS.textMuted }}>
          {mode === "cashgame" ? "Cashgame" : "Turnier"}{" — "}
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: "20px" }}>
        <div style={{ fontSize: "11px", fontWeight: "600", letterSpacing: "0.1em", textTransform: "uppercase", color: accentText, marginBottom: "4px" }}>
          Sessions
        </div>
      ...
      {/* ── API-Fehler ── */}
      {apiError && (
        <div style={{ fontSize: "12px", color: COLORS.redText, background: COLORS.redMuted, borderRadius: "8px", padding: "10px 12px", marginBottom: "14px" }}>
          ⚠ {apiError}
        </div>
      )}
      ...
      {/* ── Modus-Toggle ── */}
      <div style={{ marginBottom: "20px" }}>
        <div style={{ fontSize: "11px", fontWeight: "600", letterSpacing: "0.08em", textTransform: "uppercase", color: COLORS.textDim, marginBottom: "8px", display: "flex", alignItems: "center", gap: "6px" }}>
          <ChipsIcon /> Spielmodus
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", background: COLORS.surface, borderRadius: "10px", padding: "4px", border: `1px solid ${COLORS.border}` }}>
          {[
            ...
            return (
              ...
              >
                <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: active ? m.color : COLORS.textDim, display: "inline-block", flexShrink: 0 }} />
                {m.label}
              </button>
            );
          })}
      ...
      {/* ── Plattform (Dropdown aus API) ── */}
      <FormField label="Plattform" icon={<PlatformIcon />} hint="(optional)">
        <div style={{ position: "relative" }}>
          <select
            value={platform}
            onChange={(e) => setPlatform(e.target.value)}
            style={{ ...selectStyle, color: platform ? COLORS.text : COLORS.textMuted }}
          >
          ...
          <span style={{ position: "absolute", right: "12px", top: "50%", transform: "translateY(-50%)", color: COLORS.textDim, pointerEvents: "none", fontSize: "10px" }}>▼</span>
        </div>
      </FormField>

      {/* ── Divider ── */}
      <div style={{ borderTop: `1px solid ${COLORS.border}`, margin: "4px 0 16px", display: "flex", alignItems: "center", gap: "10px" }}>
        <span style={{ marginTop: "-1px", fontSize: "10px", fontWeight: "600", letterSpacing: "0.08em", textTransform: "uppercase", color: accentText, background: COLORS.card, padding: "0 8px" }}>
      ...
      {mode === "tournament" && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
            <FormField label="Rebuys €" icon={<EuroIcon />} hint="(gesamt)">
              <EuroInput
                value={rebuys}
            ...
            <FormField label="Add-ons €" icon={<EuroIcon />} hint="(gesamt)">
              <EuroInput
                value={addons}
      ...
      <FormField label="Notizen" icon={<NoteIcon />} hint="(optional)">
        <textarea
          placeholder={mode === "cashgame" ? "z.B. gute Tischauswahl, Tilt-Control…" : "z.B. 3. Platz, Final-Table, Bad Beat…"}
          rows={2} value={notes}
      ...
      {/* ── Profit-Vorschau ── */}
      {showPreview && (
        <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: "8px", padding: "10px 14px", marginBottom: "16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
            <span style={{ fontSize: "12px", color: COLORS.textMuted }}>Ergebnis dieser Session</span>
            <span style={{ fontSize: "17px", fontWeight: "800", color: profit >= 0 ? COLORS.greenText : COLORS.redText, display: "flex", alignItems: "center", gap: "5px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            {mode === "cashgame" ? (
              <>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>Buy-in <strong style={{ color: COLORS.textMuted }}>€{parseNum(buy_in).toFixed(2)}</strong></span>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>→</span>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>Cash-out <strong style={{ color: COLORS.textMuted }}>€{parseNum(cash_out).toFixed(2)}</strong></span>
              </>
            ) : (
              <>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>Kosten <strong style={{ color: COLORS.textMuted }}>€{(parseNum(buy_in) + parseNum(fee) + parseNum(rebuys) + parseNum(addons)).toFixed(2)}</strong></span>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>→</span>
                <span style={{ fontSize: "11px", color: COLORS.textDim }}>Gewinn <strong style={{ color: COLORS.textMuted }}>€{parseNum(winnings).toFixed(2)}</strong></span>
              </>
            )}
          </div>
        </div>
